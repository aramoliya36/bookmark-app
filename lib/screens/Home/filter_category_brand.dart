...
class FilterCategoryBrand extends StatefulWidget {
  final String selectedBrand;

  const FilterCategoryBrand(this.selectedBrand);

  @override
  _FilterCategoryBrandState createState() => _FilterCategoryBrandState();
}

class _FilterCategoryBrandState extends State<FilterCategoryBrand> {
  ...
  Future getFilterCategoryListData() async {
    ...
    dynamic response = await CategoryAPI.getFilterCategory(
        ...
        filterType: 'brand',
        filterSlug: "${widget.selectedBrand}",
        filterMap: '',
        page: "0",
        perPage: "15",
        isFilterData: "NO");
    FilterCategoriesResponse filterCategoriesResponse =
        FilterCategoriesResponse.fromJson(response);

    commanGextController.isProductCount.value = filterCategoriesResponse.count;
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            if (snapshot.data == null) {
              ...
              FilterCategoriesResponse filterCategoriesResponse = snapshot.data;

              if (filterCategoriesResponse.listResponseFilterCategory.length <=
                  0) {
                return Center(
                  child: Text(
                    'No Products !',
                  ...
                  children: [
                    Stack(
                      children: [
                        CachedNetworkImage(
                          imageUrl:
                              '${filterCategoriesResponse.listResponseFilterCategory[0].brand[0].brandImg}',
                          fit: BoxFit.cover,
                            ...
                            children: [
                              Expanded(
                                ...
                                child: Container(
                                  width: width / 2,
                                  child: Text(
                                    '${filterCategoriesResponse.listResponseFilterCategory[0].brand[0].brandName}',
