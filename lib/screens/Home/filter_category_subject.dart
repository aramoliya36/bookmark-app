...
class _FilterCategorySubjectState extends State<FilterCategorySubject> {
  ...
  Future getFilterCategoryListData() async {
    ...
    dynamic response = await HomePageApi.filterCategoryProduct(
        ...
        filterType: 'subject',
        filterSlug: "${widget.selectedSubject}",
        filterMap: '',
        page: "0",
        perPage: "15",
        isFilterData: "NO");
    // userId.toString(), 'class', widget.selectedClass);
    FilterSubjectCategoryModel _filterCategoryModel =
        FilterSubjectCategoryModel.fromJson(response);
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            FilterSubjectCategoryModel _filterCategoryModel = snapshot.data;
                ...
                children: [
                  Stack(
                    children: [
                      CachedNetworkImage(
                        imageUrl:
                            '${_filterCategoryModel.response[0].subject[0].subjectImg}',
                        // '${_filterCategoryModel.response[0].responseClass[0].classImg}',
                        fit: BoxFit.cover,
                          ...
                          children: [
                            Expanded(
                              ...
                              child: Container(
                                width: width / 2,
                                child: Text(
                                  '${_filterCategoryModel.response[0].subject[0].subjectName}',
                              ...
                              onTap: () {
                                Get.to(FilterSettingScreen(
                                  ...
                                  _filterCategoryModel
                                      .response[0].subject[0].subjectSlug,
                                  "subject"
                                ]);
                              },
                              child: Padding(
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Obx(() {
                        return commanGextController.isProductCount.value > 0
                            ? PagewiseGridView<ProductSubject>.count(
                                key: UniqueKey(),
                                pageSize: PAGE_SIZE,
                                crossAxisCount: 2,
                                mainAxisSpacing: 10,
                                ...
                                pageFuture: (pageIndex) {
                                  print("IS FUTURE CALL ELSE");
                                  return HomePageApi.getProductHomeSubject(
                                      context,
                                      pageIndex * PAGE_SIZE,
                                      PAGE_SIZE,
  ...
  Widget _itemBuilder(context, ProductSubject product, _) {
    print("IS PRODUCT CALL ");
